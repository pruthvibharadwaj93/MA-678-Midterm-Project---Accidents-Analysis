...
This project aims to understand the relationship of daily frequency of accidents with temporal and demographic variables at a county level in the 6 states of the New England region in the USA. A generalized multilevel mixed effects model is used to model the frequency of accidents in the different counties of the region. The implications and limitations for choosing multilevel mixed effects model are discussed. This report offers only directional inferences to explain fluctuation in frequency of accidents. Next steps and additional layers of complexity that can be incorporated in accident frequency prediction are also suggested towards the end.
...
In this project, I am interested in finding the relationship between daily frequency of accidents with th
...
accidents_ne_agg_g$perc_commuters <- (accidents_ne_agg_g$Number.of.workers.from.other.counties.who.commute.to.work.in.the.county + accidents_ne_agg_g$Number.of.resident.workers.who.commute.to.work.in.other.counties + accidents_ne_agg_g$Number.of.resident.workers.who.commute.within.county)/accidents_ne_agg_g$POPESTIMATE

accidents_ne_agg_g$commuters <- (accidents_ne_agg_g$Number.of.workers.from.other.counties.who.commute.to.work.in.the.county + accidents_ne_agg_g$Number.of.resident.workers.who.commute.to.work.in.other.counties + accidents_ne_agg_g$Number.of.resident.workers.who.commute.within.county)
...
simulationOutput1 <- simulateResiduals(fittedModel = ne_model_5)
...
```
```{r}
ne_model_1 <- glmer(n ~ log(POPESTIMATE)  + log(Number.of.business.establishments) + log(Median_Household_Income_2019) + wday+ Percent.of.resident.workers + Percent.of.resident.workers.who.work.at.home + Percent.of.resident.workers.who.commute.within.county + Percent.of.resident.workers.who.commute.to.work.in.other.counties + Percent.of.workers.from.other.counties.who.commute.to.work.in.the.county + Percent.of.resident.workers.who.commute.by.transit + (1+wday|County) , data = accidents_ne_agg_g, family = poisson(link = "log"))
...
```
```{r}
ne_model_4 <- glmmTMB(n ~ wday + (1+wday|County) , data = accidents_ne_agg_g, family = poisson(link = "log"))
summary(ne_model_4)
```
